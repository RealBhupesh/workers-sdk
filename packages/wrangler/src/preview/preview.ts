...
async function ensurePreviewsConfig(
	...
	const baseConfig = await getPreviewBaseConfig(config, accountId, workerName);
	...
	if (config.configPath === undefined || isNonInteractiveOrCI()) {
		throw missingPreviewsConfigError(previews, config.configPath);
	}
	...
	try {
		experimental_patchConfig(config.configPath, { previews }, false);
	} catch (error) {
